...
def build_websocket_tab(window) -> None:
    ...
    window.obs_add_target_button = QPushButton("Add Target")
    window.obs_add_target_button.clicked.connect(window.add_obs_target_source_from_combo)
    source_row = QHBoxLayout()
    source_row.addWidget(window.obs_target_source, 1)
    source_row.addWidget(window.obs_refresh_sources_button)
    source_row.addWidget(window.obs_add_target_button)
    auto_refresh_form.addRow("Source", _wrap_layout(source_row))
    window.obs_target_badges = QWidget()
    window.obs_target_badges_layout = QHBoxLayout(window.obs_target_badges)
    window.obs_target_badges_layout.setContentsMargins(0, 0, 0, 0)
    window.obs_target_badges_layout.setSpacing(6)
    auto_refresh_form.addRow("Targets", window.obs_target_badges)
    ...
    window.obs_refresh_now_button = QPushButton("Refresh Targets Now")
